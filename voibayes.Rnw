...
\if1\blind
{
  ...
  \author{Christopher Jackson, Anne Presanis, Stefano Conti, Daniela De Angelis\thanks{
    This work was funded by the Medical Research Council, grant code U105260566.  Thanks to Public Health England for providing the data.}\hspace{.2cm}\\
...
Po(\mu_M)$, with $y_M=\Sexpr{soph[2]}$. We assume some reporting bias of unknown direction,
...
adjustment, $\mu_{D} = \mu_{DG} + \mu_{DN} + \mu_{DP}$ is the
