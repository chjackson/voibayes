...
reported to the HIV and AIDS Reporting System (HARS)
\citep{PHEreport2016}.  From the 2012 version of this dataset, known
then as SOPHID (Surveillance of Prevalent HIV Infections Diagnosed),
we obtain the reported number of HIV diagnoses for MSM, $y_M \sim
Po(\mu_M)$, with $y_M=\Sexpr{soph[2]}$. We assume some reporting bias
of unknown direction, through $\log(\mu_M) = a_S + \log(\mu_D)$ where
